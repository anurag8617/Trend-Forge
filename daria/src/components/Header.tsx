...
export default function Header({ sidebarOpen, setSidebarOpen }: { sidebarOpen: boolean; setSidebarOpen: (v: boolean) => void }) {
  ...
  const { tenant, setTenant, isAlertActive, presentationMode, togglePresentationMode } = useAppState();
    ...
    <header className="h-16 border-b border-border bg-background flex items-center justify-between px-4 md:px-6 shrink-0 relative z-40">
      ...
      <div className="flex items-center gap-4 md:gap-8">
        ...
        <button
          ...
          className={`p-2 rounded-full transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400 ${
            presentationMode ? 'text-cyan-400 bg-cyan-400/10 hover:bg-cyan-400/20' : 'text-gray-500 hover:text-white hover:bg-surface'
          }`}
